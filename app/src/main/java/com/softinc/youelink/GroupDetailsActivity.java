...
public class GroupDetailsActivity extends BaseActivity implements OnClickListener {
    ...
    private Button clearcharrecordBtn;
...
//        st = getResources().getString(R.string.people);
        ...
        clearcharrecordBtn =(Button)findViewById(R.id.clear_chat_record);
...
//        ((TextView) findViewById(R.id.group_name)).setText(group.getGroupName() + "(" + group.getAffiliationsCount() + st);
        ((TextView)findViewById(R.id.group_ID_TextView)).setText(groupId);
        ((TextView)findViewById(R.id.group_count)).setText(group.getAffiliationsCount() + "/" + group.getMaxUsers());
        ((TextView) findViewById(R.id.group_name)).setText("群聊设置");
...
//        blacklistLayout.setOnClickListener(this);
    ...
    private void loadGroupListInfo() {
        RequestParams params = new RequestParams();
        params.put("chatGroup", groupId);
...
//                        Log.e("chat","grouplist-----usern-->"+tem.toString());
...
//                Log.e("CHAT", "onFailure---------->" + responseString);
            ...
            public void run() {
                try {
                    ...
                    if (EMChatManager.getInstance().getCurrentUser().equals(group.getOwner())) {
                        ...
                        List<String> array=EMGroupManager.getInstance().getGroup(groupId).getMembers();//获取群组成员

                        String nicknameString ="";
                        for (int i=0;i<array.size();i++){
                            String hxname =array.get(i);
//                                        nicknameString =nicknameString+user
                            for (User t :groupUserList){
                                if (hxname.equals(t.getUsername())){

                                    nicknameString =nicknameString +t.getNick();
                                }
                        ...
                        EMGroupManager.getInstance().changeGroupName(groupId,nicknameString);
...
//                                    + st);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
                            ...
                            try {
                                ...
                                runOnUiThread(new Runnable() {
                                    public void run() {
                                        ...
                                        // 如果自己是群主，显示解散按钮
                                        if (EMChatManager.getInstance().getCurrentUser().equals(group.getOwner())) {
                                            Toast.makeText(getApplicationContext(), "建群人 不能屏蔽群消息", 1).show();
                                        }else{
                                            Toast.makeText(getApplicationContext(), "您已经进行过屏蔽群操作 再次操作需要群组权限", 1).show();
                                        }
            ...
            case R.id.clear_chat_record: // 清空聊天记录
                String st9 = getResources().getString(R.string.sure_to_empty_this);
                Intent intent = new Intent(GroupDetailsActivity.this, AlertDialog.class);
                intent.putExtra("cancel", true);
                intent.putExtra("titleIsCancel", true);
...
//				button.setCompoundDrawables(null, avatar, null, null);
                ...
                for (User temp:groupUserList){
                    if (temp.getUsername().equals(username)){
                            holder.textView.setText(temp.getNick());
